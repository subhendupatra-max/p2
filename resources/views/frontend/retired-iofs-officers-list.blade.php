                ...
                <div class="gallary-sec">
                    <div class="container">
                        <div class="gallary-sec-row">
                                                ...
                                                <div class="form-item-1">
                                                    ...
                                                    <select name="grade" id="grade"
                                                        ...
                                                        @forelse ($grades as $grade)
                                                            <option value="{{ $grades }}"
                                                                {{ request()->query('grade') == $grades ? 'selected' : '' }}>
                                                                {{ $grade }}</option>
                                                        @empty
                                                        @endforelse
                                                    </select>
                            ...
                            <div><p>{{ __('messages.last_updated_date') }}  : {{ date('d-m-Y', strtotime($retired_iofs_officers->first()->created_at)) }}</p></div>
                            <div class="row">
                                ...
                                <table class="table table-bordered">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>{{ __('messages.date_of_joining_iofs') }}</th>
                                            <th>{{ __('messages.date_of_retirement') }}</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @forelse($retired_iofs_officers as $officer)
                                            <tr>
                                                <td>{{ $loop->iteration }}</td>
                                                <td>{{ $officer->unit ?? 'N/A' }}</td>
                                                <td>{{ $officer->pno ?? 'N/A' }}</td>
                                                <td>{{ $officer->name ?? 'N/A' }}</td>
                                                <td>{{ $officer->grade ?? 'N/A' }}</td>
                                                <td>{{ date('d-m-Y', strtotime($officer->doj_iofs)) ?? 'N/A' }}</td>
                                                <td>{{ date('d-m-Y', strtotime($officer->dor)) ?? 'N/A' }}</td>
                                            </tr>
                                        @empty
                                            <tr>
                                  ...
                                  {!! $retired_iofs_officers->appends(request()->all())->links('pagination::bootstrap-5') !!}
                            </div>
                        </div>
                    </div>
                </div>
