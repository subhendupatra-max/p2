                                                ...
                                                <div class="col-lg-4 col-md-12 col-12">
                                                    <div class="form-item-1">
                                                        ...
                                                        <select name="sort_by" id="sort_by"
                                                            ...
                                                            <option value="3"
                                                                {{ request()->query('sort_by') == '3' ? 'selected' : '' }}>
                                                                {{ __('messages.publish_date_sort_by_asc') }}</option>
                                                            <option value="4"
                                                                {{ request()->query('sort_by') == '4' ? 'selected' : '' }}>
                                                                {{ __('messages.publish_date_sort_by_desc') }}</option>
                                ...
                                <table class="table table-bordered">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>{{ __('messages.attachment_file') }}</th>
                                            <th>{{ __('messages.file_language') }}</th>
                                            <th>{{ __('messages.published_date') }}</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @forelse($reportDatas as $reportData)
                                            <tr>
                                                <td>{{ $loop->iteration }}</td>
                                                <td>{{ localized_field($reportData, 'title') ?? 'N/A' }}</td>
                                                <td><a href="{{ $reportData->file }}" download><i
                                                            class="fa fa-download"></i></a></td>
                                                <td>{{ $reportData->file_language ?? 'N/A' }}</td>
                                                <td>{{ $reportData->public_date ?? 'N/A' }}</td>
                                            </tr>
                                        @empty
                                            <tr>
